...
! Calculate Rcut & block list QM-MM
...
!iquench lo dejamos como 0, luego cambiar
               ...
               if(idyn .gt. 3 .or. feopt) then
                  if(.not. feopt) then
                     write(6,999) 'hybrid: Temperature Antes:', tempion, ' K'
                    call calculateTemp(Ekinion,tempion,tempqm,vat,ntcon, &
                         nfree,cmcf)
                     write(6,999) 'hybrid: Kinetic Energy (eV):',Ekinion/eV
                     write(6,999) 'hybrid: Total Energy + Kinetic (eV):',(Etots+Ekinion)/eV
                     write(6,999) 'hybrid: System Temperature:', tempion, ' K'
                     if(qm) write(6,999) &
...
!Write Energy in file
               if (.not. feopt) call wriene(step,slabel,idyn,Etots,cfmax)
...
!guardo en rce y rcg.
            call wrirtc(slabel,Etots,dble(replica_number),replica_number, &
                 na_u,nac,natot,rclas,atname,aaname,aanum,nesp,atsym,isa)
